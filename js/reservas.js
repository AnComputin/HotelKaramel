...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Usuario actual: si hay sesión, precargar nombre y correo (solo lectura)
  ...
  // Copia local de trabajo: viene de la fuente única de verdad (habitaciones-data.js)
  const listaHabitaciones = obtenerHabitacionesConDisponibilidad();

  // Estado del descuento y del total (se actualizan en cada recálculo)
  let porcentajeDescuento = 0;
  let totalActual = 0; // ← ya no hace falta parsear el texto del DOM al guardar la reserva
  ...
  function calcularTotal() {
    ...
    totalActual = granTotal; // ← se guarda el número real, ya no hay que parsear texto luego
  ...
  function guardarReservaUsuario(reserva) {
    const reservasGuardadas = JSON.parse(localStorage.getItem("reservas_karamel") || "[]");
    reservasGuardadas.push(reserva);
    localStorage.setItem("reservas_karamel", JSON.stringify(reservasGuardadas));
  }
  ...
  formReserva.addEventListener("submit", (evento) => {
    ...
    const habIndex = listaHabitaciones.findIndex((h) => h.tipo === selectHabitacion.value);
    const habitacionSeleccionada = listaHabitaciones[habIndex];
    ...
    checkServicios.forEach((check) => {
      if (check.checked) {
        ...
        serviciosSeleccionados.push({
          ...
          precioUnitario,
          cantidad: huespedes,
          subtotal: precioUnitario * huespedes,
        });
      }
    });
    ...
    const nuevaReserva = {
      ...
      imagen: habitacionSeleccionada.imagen,
      fechaEntrada: inputFechaEntrada.value,
      fechaSalida: inputFechaSalida.value,
      noches,
      huespedes,
      servicios: serviciosSeleccionados,
      descuento: porcentajeDescuento,
      total: totalActual, // ← número calculado en calcularTotal(), ya no se parsea del DOM
      estado: "Confirmada",
      fechaReserva: new Date().toISOString(),
    };

    guardarReservaUsuario(nuevaReserva);

    // Descontar disponibilidad y persistirla
    listaHabitaciones[habIndex].disponibles -= 1;
    const disponibilidad = {};
    listaHabitaciones.forEach((h) => {
      disponibilidad[h.tipo] = h.disponibles;
    });
    localStorage.setItem("disponibilidad_karamel", JSON.stringify(disponibilidad));

    // Modal de confirmación
    document.getElementById("confirm-nombre").textContent = usuario.nombre;
    document.getElementById("confirm-email").textContent = usuario.email;
    document.getElementById("confirm-habitacion").textContent = habitacionSeleccionada.nombre;
    document.getElementById("confirm-fechas").textContent = `${inputFechaEntrada.value} al ${inputFechaSalida.value}`;
    document.getElementById("confirm-total").textContent = resPrecioTotal.textContent;
  ...
  // Configurar botón 'Volver al Inicio' (antes era inline onclick en HTML)
