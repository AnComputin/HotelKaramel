...
document.addEventListener("DOMContentLoaded", () => {
  ...
  formReserva.addEventListener("submit", (evento) => {
    ...
    // Persistir disponibilidad restante (por tipo) para esta sesión del navegador
    const disponibilidad = {};
    listaHabitaciones.forEach((h) => { disponibilidad[h.tipo] = h.disponibles; });
    localStorage.setItem("disponibilidad_karamel", JSON.stringify(disponibilidad));

    // Llenar el modal de confirmación
    document.getElementById("confirm-nombre").textContent = inputNombre.value.trim();
    document.getElementById("confirm-email").textContent = inputEmail.value.trim();
    document.getElementById("confirm-habitacion").textContent = listaHabitaciones[habIndex].nombre;
